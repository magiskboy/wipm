...
NS = Namespace("dataset", description="")
...
class Dataset(Resource):
    ...
    @NS.marshal_with(res_dataset.CREATE_DATASET_RES)
    def post(self, **kwargs):
        """Create dataset for push data
        """
        id_dataset = serv_dataset.create_dataset()
        return {"message": "Create dataset success", "id": str(id_dataset)}

    @NS.expect(req_dataset.PUSH_DATASET_PARAMS, validate=True)
    @NS.marshal_with(res_dataset.BASE_RES)
